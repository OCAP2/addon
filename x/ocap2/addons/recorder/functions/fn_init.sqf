...
GVAR(shouldSave) = [false, true] select (EGVAR(settings,minMissionTime) < 10);

if (ocap_excludeMarkerFromRecord isEqualType []) then {
  publicVariable QEGVAR(settings,excludeMarkerFromRecord);
} else {
...
0 spawn {
  if (GVAR(shouldSave)) exitWith {};
  LOG(["Waiting freeze end!"]);
  waitUntil {sleep 1.4};
  LOG(["Waiting until EGVAR(settings,minMissionTime) ends"]);
  sleep EGVAR(settings,minMissionTime);
  LOG(["Recording duration minimum has been exceeded. will save."]);
  GVAR(shouldSave) = true;
};
